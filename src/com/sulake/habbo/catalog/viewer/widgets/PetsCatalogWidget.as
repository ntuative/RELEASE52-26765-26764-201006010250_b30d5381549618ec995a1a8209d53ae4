package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IAvatarImageListener
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(_selectedColorIndex >= var_294.length)
         {
            return "";
         }
         var petColor:IPetColor = var_294[_selectedColorIndex] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_565[var_1430] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(_selectedColorIndex < 0 || _selectedColorIndex > var_294.length)
         {
      ...
      override public function init() : void
      {
         ...
         var_800 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_294 = [];
         var_565 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_800,this);
               if(_loc11_)
               {
                  var_565 = _loc11_.breeds;
                  var_294 = _loc11_.colors;
                  if(var_294 == null)
                  {
                     var_294 = [];
                  }
         ...
         while(_loc6_ < var_565.length)
         {
            ...
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_800,_loc12_.id),getRaceLocalizationKey(var_800,_loc12_.id)));
            _loc6_++;
         }
         var_581.add(var_800,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_890,onApproveNameResult);
         _loc5_ = var_581.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_294)
         {
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_581.getWithIndex(0),page,p);
      }
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_581.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(_selectedColorIndex >= 0 && _selectedColorIndex < var_294.length)
         {
            _loc7_ = var_294[_selectedColorIndex] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_565[var_1430] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(var_800,_loc3_.id,_loc2_,AvatarScaleType.const_55,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_291,AvatarAction.const_557);
